...
PointMatcher::PointMatcher():eps(0){}

PointMatcher::PointMatcher(const double eps):eps(eps){}

double PointMatcher::computeEuclide(const Descriptor &d1, const Descriptor &d2) const
{
    double v = 0;
    for(int i = 0; i<d1.data.size(); i++){
        v += (d1.data[i]-d2.data[i])*(d1.data[i]-d2.data[i]);
    }
    return sqrt(v);
}
